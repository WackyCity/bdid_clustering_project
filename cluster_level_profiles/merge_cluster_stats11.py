import pandas as pd
import os

# Directories (update paths as needed)
input_csv_dir = 'intercluster_outputs'  # Update as needed
stats_dir = 'output_files/former_without_intercluster'      # Update as needed
output_dir = 'new_aggregate_w_intercluster'

os.makedirs(output_dir, exist_ok=True)

# Clustering method identifiers
keys = ['0.001', '0.005', '0.01', '0.05', '0.1', '0.2', 'modularity']

# Helper function to detect file type based on extension
def get_separator(file_name):
    if file_name.endswith('.csv'):
        return ','
    elif file_name.endswith('.tsv'):
        return '\t'
    else:
        return None

for key in keys:
    # List files matching the key for .csv or .tsv extensions
    input_files = [f for f in os.listdir(input_csv_dir) if key in f and (f.endswith('.csv') or f.endswith('.tsv'))]
    stats_files = [f for f in os.listdir(stats_dir) if key in f and (f.endswith('.csv') or f.endswith('.tsv'))]

    # Debug: print the files found for the current key
    print(f"Files for key '{key}':")
    print(f"Input files: {input_files}")
    print(f"Stats files: {stats_files}")

    if not input_files or not stats_files:
        print(f"[{key}] Missing input or stats file.")
        continue

    input_path = os.path.join(input_csv_dir, input_files[0])
    stats_path = os.path.join(stats_dir, stats_files[0])

    # Step 1: Read input file with headers from row 0
    input_sep = get_separator(input_files[0])  # Detect separator for input file
    df_input = pd.read_csv(input_path, sep=input_sep, header=0)

    # Step 2: Get correct name for first column from row 3 (index 2)
    with open(input_path, 'r') as f:
        row3 = f.readlines()[2]
    first_col_name = row3.split(input_sep)[0].strip()

    # Step 3: Set the first column's name manually
    df_input.columns = [first_col_name] + df_input.columns[1:].tolist()
    df_input.columns = df_input.columns.str.strip().str.lower()

    # Step 4: Ensure first column is named 'cluster_id'
    if df_input.columns[0] != 'cluster_id':
        df_input.rename(columns={df_input.columns[0]: 'cluster_id'}, inplace=True)

    # Step 5: Drop old count columns if they exist (node_count, edge_count)
    df_input = df_input.drop(columns=[col for col in ['node_count', 'edge_count'] if col in df_input.columns])

    # Step 6: Read and normalize the stats file
    stats_sep = get_separator(stats_files[0])  # Detect separator for stats file
    df_stats = pd.read_csv(stats_path, sep=stats_sep)

    # Normalize column names from stats (to match lowercase format)
    df_stats.columns = df_stats.columns.str.strip().str.lower()

    # Step 7: Merge the entire stats file into the cluster file, including all columns
    df_input['cluster_id'] = df_input['cluster_id'].astype(str)
    df_stats['cluster_id'] = df_stats['cluster_id'].astype(str)

    # Merge all columns from stats into the input file
    df_merged = df_input.merge(df_stats, on='cluster_id', how='left')

    # Step 8: Write output to CSV (force .csv extension)
    output_filename = input_files[0].replace(os.path.splitext(input_files[0])[1], '.csv')  # Change extension to .csv
    output_path = os.path.join(output_dir, output_filename)
    df_merged.to_csv(output_path, sep=',', index=False)  # Save as CSV (comma-separated)

    print(f"[{key}] Merged and saved to {output_path}")
